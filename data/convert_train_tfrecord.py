...
flags.DEFINE_string("dataset_path", "../Dataset/ms1m_align_112/imgs", "path to dataset")
flags.DEFINE_string("output_path", "./data/ms1m.tfrecord", "path to ouput tfrecord")
    ...
    with tf.io.TFRecordWriter(FLAGS.output_path) as writer:
        for id_name, img_path in tqdm.tqdm(samples):
            tf_example = make_example(
                source_id=int(id_name), img_path=str.encode(img_path)
            )
